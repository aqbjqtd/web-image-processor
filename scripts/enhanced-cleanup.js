  ...
  findProcessByPort() {
    try {
      if (process.platform === 'win32') {
        const result = execSync(`netstat -ano | findstr :${this.port} | findstr LISTENING`, { encoding: 'utf8' });
        ...
        const result = execSync(`lsof -t -i:${this.port}`, { encoding: 'utf8' });
  ...
  getProcessInfo(pid) {
    try {
      if (process.platform === 'win32') {
        ...
        const result = execSync(`ps -p ${pid} -o pid,ppid,cmd,etime`, { encoding: 'utf8' });
        const lines = result.trim().split('\n').filter(line => line.trim());
        if (lines.length > 1) {
    ...
    try {
      if (process.platform === 'win32') {
        ...
        lines.forEach((line, index) => {
          ...
          console.log(`   ${index + 1}. ${parts[0]} (PID: ${parts[1]}, 内存: ${parts[4]} KB)`);
        });
        ...
        try {
          ...
          console.log(`📝 发现 ${pids.length} 个Node.js进程: ${pids.join(', ')}`);
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    default:
      console.log('使用方法:');
      console.log('  node scripts/enhanced-cleanup.js port [端口号]  - 清理指定端口');
      console.log('  node scripts/enhanced-cleanup.js all             - 清理所有Node进程');
      console.log('  node scripts/enhanced-cleanup.js full [--all]    - 完整清理流程');
      console.log('');
      console.log('示例:');
